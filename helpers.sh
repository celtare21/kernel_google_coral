...
function mkzip() {
    ...
    curl https://bashupload.com/Artemis.zip --data-binary @Artemis.zip
    cd ../pixel4
}

function mkimg() {
    echo "Making a new image"
    ../../mkbootimg/mkbootimg.py  --kernel out/arch/arm64/boot/Image.lz4-dtb --ramdisk ../../R-stock/ramdisk --dtb out/arch/arm64/boot/dts/google/qcom-base/sm8150-v2.dtb --cmdline 'console=ttyMSM0,115200n8 androidboot.console=ttyMSM0 printk.devkmsg=on msm_rtb.filter=0x237 ehci-hcd.park=3 service_locator.enable=1 androidboot.memcg=1 cgroup.memory=nokmem usbcore.autosuspend=7 androidboot.usbcontroller=a600000.dwc3 swiotlb=2048 androidboot.boot_devices=soc/1d84000.ufshc loop.max_part=7 buildvariant=user' --header_version 2 -o ../new.img
