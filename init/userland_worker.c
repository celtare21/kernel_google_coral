...
static void userland_worker(struct work_struct *work)
{
	...
	strcpy(argv[1], "-c");
	strcpy(argv[2], "/system/bin/echo 0 > /sys/devices/system/cpu/cpu7/online");
	argv[3] = NULL;

	ret = use_userspace(argv);
	if (!ret)
		pr_info("Echo called succesfully!");
	else
		pr_err("Couldn't call echo! %d", ret);

	strcpy(argv[0], "/system/bin/chmod");
	strcpy(argv[1], "444");
	strcpy(argv[2], "/sys/devices/system/cpu/cpu7/online");
	argv[3] = NULL;
